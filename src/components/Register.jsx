    ...
    <div className="container">
        <div className="row justify-content-center">
           ...
           <h3 className='text-center mb-2'>Sign Up</h3>
            <p className='text-center text-muted login_description mb-4'>Create your account to get full access</p>
            <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
            <Form.Label className='fs-5 text-start text-muted'>User Name</Form.Label>
            ...
            {errors?.userName?.type==="pattern" && (<h6 className='text-danger' >Please enter username without spaces, starting with a capital letter.</h6>)}
            </Form.Group>
            ...
            {errors?.email?.type==="pattern" && (<h6 className='text-danger' >email is not valid .</h6>)}
            </Form.Group>
            ...
            <Form.Label className='fs-5 text-muted'>Confirm Password</Form.Label>
            <Form.Control type="password" name="confirmPassword" {...register("confirmPassword" ,{
              required:true,
              validate: (value) => value === myPassword || 'Passwords do not match',

            })}  />
            {errors?.confirmPassword?.type==="required" && (<h6 className='text-danger' >field is required .</h6>)}
            {errors?.confirmPassword?.message && (<h6 className='text-danger' >{errors?.confirmPassword?.message}</h6>)}
            </Form.Group>
            <div className='d-flex justify-content-center my-4' >
            <button className='btn login_btn' type='submit'>Sign Up</button>
            </div>
            <p className='text-center text-muted login_description mb-4'>Already have an account? <Link to="/login"> Login </Link> here</p>
        </Form>
           </div>
        </div>
